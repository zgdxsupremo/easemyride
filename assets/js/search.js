/**
 * MargDrive — Vehicle Search Results Controller (search.js)
 * 
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (urlParams.has("from")) {
    ...
    try {
      searchData = JSON.parse(localStorage.getItem(MargDriveConfig.storageKeys.lastSearch));
    } catch (e) {
      searchData = null;
    }
  }
  ...
  if (modifyForm) {
    modifyForm.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        localStorage.setItem(MargDriveConfig.storageKeys.lastSearch, JSON.stringify(updated));

 ...
 * 
 * Provides unified, structured city searching, alias matching, state identification,
 * and high-altitude / regional metadata lookup for all search inputs across the app.
 */
...
const CitySearch = (() => {
  ...
  // Load from india-cities.json synchronously if running in node, or via fetch in browser
  try {
  ...
  /**
   * Initializes / fetches the full JSON dataset asynchronously
   */
  async function init() {
  ...
  // Self-init in browser
  if (typeof window !== "undefined") {
    init();
  }

  /**
   * Searches cities by name, state, or alias with fuzzy match ranking.
   * @param {string} query 
  ...
  /**
   * Retrieves a structured city object by name or alias.
  ...
  /**
   * Binds autocomplete to an input field element.
  ...
  function attachAutocomplete(inputEl, onSelect) {
    ...
    function renderSuggestions(results) {
      ...
      dropdown.innerHTML = results.map((city, idx) => `
        <div class="city-suggest-item" data-index="${idx}" style="padding:10px 14px; cursor:pointer; border-bottom:1px solid #F1F5F9; display:flex; justify-content:space-between; align-items:center; transition:background 0.15s;">
          <div>
            <div style="font-weight:600; color:#0F172A; font-size:0.95rem;">${city.name}</div>
            <div style="font-size:0.78rem; color:#64748B;">${city.state} (${city.region} India)</div>
          </div>
          ${city.terrainCategory === "HIGH_ALTITUDE" ? '<span style="font-size:0.7rem; background:#FEF3C7; color:#B45309; padding:2px 6px; border-radius:10px; font-weight:700;">⛰️ Hill / Dham</span>' : ''}
          ${city.terrainCategory === "AIRPORT_HUB" ? '<span style="font-size:0.7rem; background:#E0F2FE; color:#0369A1; padding:2px 6px; border-radius:10px; font-weight:700;">✈️ Airport</span>' : ''}
        </div>
      `).join("");
...
fs.writeFileSync(path.join(__dirname, 'assets', 'js', 'city-search.js'), code, 'utf8');
console.log('Successfully wrote city-search.js');
